            ...
            <div class="flex">
                <div class="cart">
                    <button class="flex items-start py-2" @click="cartDrawerOpen=true">
                        ...
                        <div id="header-cart-badge" class="flex justify-center items-center w-4 h-4 bg-secondary font-bold text-xs text-primary rounded-md">{{ $cartItems->count() }}</div>
                    </button>
                </div>
                @if(auth()->check() && (auth()->user()->hasRole('normal') || auth()->user()->hasRole('wseller')))
                    <div class="user ml-4 relative" style="z-index: 999999;" x-data="{userDropdownOpen:false}">
                        ...
                        <div x-cloak x-show="userDropdownOpen" @click.away="userDropdownOpen=false" class="user-menu absolute w-30 bg-white text-sm shadow-md " style="top:100%; right:-1rem; z-index: 9999;">
                            <ul>
                                <li><a href="" class="block px-4 py-2 border-bottom-light">My Orders</a></li>
                                <li><a href="javascript:;" onclick="event.preventDefault();document.getElementById('logout-form').submit();" class="block px-4 py-2 border-bottom-light">Logout</a></li>
