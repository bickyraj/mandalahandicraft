    ...
    <tr>
        ...
        <td class="header container" style="display:block!important;max-width:600px!important;margin:0 auto!important;clear:both!important;">
            <div class="content" style="max-width:600px;margin:0 auto;display:block;">
                <table bgcolor="#27c7d8" style="width: 100%;padding: 16px;">
                    <tr>
                        <td>
                        ...
                        <td align="right">
                            <h4 style="font-weight:600; font-size: 18px; text-transform: uppercase; color:#fff;margin:0;padding:0;font-family:HelveticaNeue-Light, Helvetica Neue Light, Helvetica Neue, Helvetica, Arial, Lucida Grande, sans-serif; line-height: 1.1; margin-bottom:15px;">Order ID: {{$order->order->order_code }}</h4>
                        </td>
                    </tr>
                </table>
            </div>
                ...
                <table style="width: 100%;">
                    <tr>
                        <td>
                            ...
                            <div class="content">
                                <p><b>Your order for product {{ucwords($order->product->title)}} is {{strtoupper($order->status)}}</b></p>
                                ...
                                <table border style="width:100%;border-collapse:collapse;">
                                  ...
                                  <tr>
                                   ...
                                   <tbody>
                                    <td>{{ucwords($order->product->title)}}</td>
                                    <td>{{!is_null($order->color)?$order->color->name:'-'}}</td>
                                    <td>{{!is_null($order->size)?$order->size->size:'-'}}</td>
                                    <td>{{$order->quantity}}</td>
                                    <td>{{$order->rate}}</td>
                                    <td>{{$order->status}}</td>
                                     <td>Rs.{{$order->quantity*$order->rate}}</td> 
                        ...
                        display:block;
                        text-align:center; background-color: #3B5998!important;" href="{{$company->facebook_url}}" class="soc-btn fb">Facebook</a>
                        ...
                        color: #FFF;font-weight:bold;
                        display:block;
                        text-align:center;background-color: #1daced!important;" href="{{$company->twitter_url}}" class="soc-btn tw">Twitter</a>
                    ...
                    <tr>
                        <td align="center">
                            <p style="margin-bottom: 13px;font-weight: normal;font-size:14px;line-height:1.6;border-top: 1px solid rgb(215,215,215); padding-top:15px;font-size:10px;font-weight: bold;">
                                <span style="font-weight:500">Copyright {{date('Y')}}  | Wearhubs</span> <br>
                                <a style="color: #2BA6CB;" href="{{route('terms')}}">Terms</a> |
