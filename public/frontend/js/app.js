	...
	GOODWIN.product = {
		...
		swatchToggle: function (option) {
			$(option).each(function (key,val) {
				...
				let currentSelect = $(".prd-color.swatches").find("li.active a").attr("data-value");
				$previewsCarousel.slick('slickUnfilter');
				$previewsCarousel.slick('slickFilter', 'a[data-value="' + 6 + '"]');
				$previewsCarousel.slick('refresh');
				$previewsCarousel.find('.slick-current').trigger('click');

				let slickTrack = $(".slick-track").find("a:first");
				slickTrack.addClass("active");

				let currentImageSrc = slickTrack.find("img").attr("src");

				$("#productPreviewImage").attr('src', currentImageSrc);
